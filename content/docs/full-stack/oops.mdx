...
1. **Procedural Programming** - Code is organized as procedures or routines. Works well for
   ...
   Why did procedural programming (like C) hit limitations?
    ...
    - Everything lived in the global namespace → tight coupling, spaghetti code.
    - Reuse was limited — “copy-paste” programming.
...
2. **Structured Programming** - Introduced control structures (if, while, for) to improve flow.
   Reduced goto usage, making code clearer.
    ...
    - Still relied on global state → tight coupling.
...
### 🧩 Abstraction — *Only tell me what I need to know*

> You drive a car with the steering wheel. You don’t care how the engine or transmission works.

Thats exactly what abstraction is about.
- Reduces cognitive overload: Manage complexity by hiding irrelevant details.
- Behavior over implementation: Focus on *what* an object does, not *how* it does it.
- Encourages loose coupling: Accommodate changes without affecting others.
- Design by contract: Design contracts / interfaces for multiple systems to interact.

<Tabs items={["java", "c++", "python"]}>
    <Tab value="java">
        ```java
...
> A bank account doesn’t let you directly set your balance — you use a method that enforces rules.
...
<Callout type="warn">
    Use inheritance only when the relationship is truly hierarchical — otherwise, prefer composition.
</Callout>
